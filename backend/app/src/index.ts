...
async function connectMongo(): Promise<Db> {
  ...
  try {
    ...
    const mong_uri: string =
      'mongodb://mongo:' + mong_pass + '@pknepps.net/?authSource=admin';
...
async function caseThree(mongo_db: Db) {
  ...
  while (!validTransaction) {
    try {
      ...
      console.log(
        'An exception happened while waiting for a response from user: '
      );
    }
  }
}
...
async function caseFour(mongo_db: Db) {
  try {
    ...
    const product_id: number = Number(
      await rl.question('Enter a valid product id: ')
    );
...
async function caseFive(mongo_db: Db) {
  try {
    const decision = (
      await rl.question('Data will be inserted into which table: ')
    ).toLowerCase();
    ...
    switch (decision) {
      ...
      case 'products':
        ...
        console.log(
          `Inserted ${quantity} random products into the PRODUCTS table.`
        );
        break;
      case 'transactions':
        randTransaction(quantity)
          ...
          .catch((e) =>
            console.log(
              'An exception has occurred while inserting into transactions: ' +
                e
            )
          );
        break;
      ...
      case 'reviews':
        randRatings(quantity)
          .then((result) => {
            ...
            console.log(
              `Added ${quantity} random ratings to users and products.`
            );
          })
          .catch((e) =>
            console.log(
        ...
        randReviews(quantity)
          .then((result) => {
            ...
            console.log(
              `Added ${quantity} random reviews to users and products.`
            );
          })
          .catch((e) =>
            console.log(
...
async function start() {
  ...
  try {
    ...
    console.log(
      'An error happened when trying to get the current transaction and product ids: ',
      e
    );
  }
}
