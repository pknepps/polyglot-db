...
export function updateUser(props: Partial<User> & Pick<User, "username">, db: mongodb.Db) {
    // @ts-ignore
    db.collection("users").updateOne({"username": props.username}, {$set: props})
...
export function updateProduct(props: Partial<Product> & Pick<Product, "product_id">, db: mongodb.Db) {
    // @ts-ignore
    db.collection("products").updateOne({"product_id": props.product_id}, {$set: props})
 ...
 */
export function addReview(review: {"username": string, "product_id": number, "review": string}, db: mongodb.Db) {
 ...
 * @param db The mongo database to update.
 */
export function addRating(rating: {"username": string, "product_id": number, "rating": number}, db: mongodb.Db) {
    db.collection("users").updateOne(
