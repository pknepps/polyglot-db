...
fn main() {
    ...
    if args.len() < 2 || args.len() > 5 {
        eprintln!("Usage: {} {{setup | teardown}} [postgres | mongodb | neo4j]", args[0]);
        process::exit(1);
    } 

    // creates a config based on input, either setup all or just one
    let perform = if args.len() == 2 {
        PerformOn::All
    } else {
        match args[2].as_str() {
            "postgres" => PerformOn::Postgres,
            "mongodb" => PerformOn::MongoDB,
            "neo4j" => PerformOn::Neo4j,
    ...
    match args[1].as_str() {
        "setup" => start(Action::Setup(perform)),
        "teardown" => start(Action::Teardown(perform)),
        _ => {
